...
int DeleteMultipleObjectsRequest::build_command_specific(HttpRequest *request) {
    // POST /?delete
    ...
    // assemble json value
    Json::Value value;
    for (std::vector<std::string>::const_iterator it = _object_keys.begin();
         it != _object_keys.end();
         ++it) {
        Json::Value obj;
        obj["key"] = Json::Value(*it);
        value["objects"].append(obj);
    }
